...
const logger = {
  log: (message: string, data?: any) => {
    ...
    console.log(`[${timestamp}] [PUBLIC_PROXY] ${message}`, data ? JSON.stringify(data, null, 2) : '');
  },
  error: (message: string, error?: any) => {
    const timestamp = new Date().toISOString();
    console.error(`[${timestamp}] [PUBLIC_PROXY_ERROR] ${message}`, error ? JSON.stringify(error, null, 2) : '');
  },
};
...
async function proxy(req: Request, ctx: any) {
  ...
  logger.log('Public proxy request initiated', {
    method: req.method,
    path,
    fullUrl: backendUrl,
