...
export default function CandidateMappingAdminTab({ backend }: { backend: string }) {
  ...
  const loadParties = async () => {
    const r = await fetch(`/v1/public/parties`);
  ...
  const loadLokSabhas = async () => {
    const r = await fetch(`/v1/public/ls`);
  ...
  const loadAssembliesByLs = async (lsCode: string) => {
    const r = await fetch(`/v1/public/assemblies/by-ls?lsCode=${lsCode}`);
  ...
  const loadCandidates = async () => {
    let url = `/v1/public/candidates?year=${year}`;
  ...
  const updateCandidateParty = async (candidateId: number, partyId: number) => {
    const r = await fetch(`/v1/admin/candidates/${candidateId}/party`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
